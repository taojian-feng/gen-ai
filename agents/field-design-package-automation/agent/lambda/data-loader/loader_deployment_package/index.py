...
EXISTING_FDPS_TABLE_NAME = os.environ.get('EXISTING_FDPS_TABLE_NAME')
...
def lambda_handler(event, context):
    ...
    if request_type == 'Create' or request_type == 'Update':
        try:
            with open('fdps.json', 'r') as file:
                fdps_data = json.load(file)
            
            items = []
            for fdp in fdps_data:
                item = {}
                for key, value in fdp.items():
                    if value:
            ...
            response = dynamodb.batch_write_item(
                RequestItems={
                    EXISTING_FDPS_TABLE_NAME: items
                }
            )
